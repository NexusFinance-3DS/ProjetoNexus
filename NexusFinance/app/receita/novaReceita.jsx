  ...
  async function salvar() {
    ...
    try {
      const response = await apiAutenticada("/financeiro/transacoes", {
        method: "POST",
        body: JSON.stringify({ tipo: "Receita", valor, descricao, data, categoria, recorrente, observacao, status: "Confirmada" }),
      });
      ...
      <KeyboardAvoidingView style={keyboardStyles.avoidingView} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={keyboardStyles.scrollContent} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets={Platform.OS === "ios"}>
          <View style={styles.addValor}>
            ...
            <TextInput style={[styles.InputValor, sharedStyles.textAlignRight]} value={valor} onChangeText={setValor} keyboardType="decimal-pad" placeholder="R$ 0,00" placeholderTextColor="#3f3f3f" />
          </View>
          <View style={styles.inputFull}><Text style={sharedStyles.formLabel}>Descrição</Text><TextInput style={styles.input} placeholder="Ex.: Salário" placeholderTextColor="#999" value={descricao} onChangeText={setDescricao} /></View>
          ...
          <View style={styles.inputFull}><Text style={sharedStyles.formLabel}>Categoria</Text><TextInput style={styles.input} placeholder="Salário" placeholderTextColor="#999" value={categoria} onChangeText={setCategoria} /></View>
          <View style={styles.listItem}>
            <View style={styles.iconBox}><Icon name="repeat" size={20} color="#fff" /></View>
          ...
          <View style={styles.saveWrapper}><TouchableOpacity style={styles.saveButton} onPress={salvar} disabled={salvando}><Text style={styles.saveButtonText}>{salvando ? "Salvando..." : "Salvar receita"}</Text></TouchableOpacity></View>
        </ScrollView>
